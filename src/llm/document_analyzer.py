...
class DocumentAnalyzer:
    ...
    def analyze_paper(
        self, pdf_path: Path, progress_callback: ProgressCallback | None = None
    ) -> PaperAnalysisSchema:
        """
        ...
        if progress_callback:
            progress_callback(ProgressStage.ANALYZING, 10, "Starting paper analysis...")
        ...
        if progress_callback:
            progress_callback(
                ProgressStage.ANALYZING, 30, "Analyzing document structure and content..."
            )
        ...
        try:
            paper_analysis = self.gemini_client.generate_structured_output(
                prompt=prompt, response_schema=PaperAnalysisSchema, pdf_path=pdf_path
            )

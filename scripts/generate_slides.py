...
from src.utils.progress import ProgressCallback, ProgressStage, create_noop_callback
...
def save_checkpoint(
    ...
    paper_analysis: PaperAnalysisSchema | None = None,
    ...
    checkpoint_data = {"step": step, "timestamp": datetime.now().isoformat(), "data": data or {}}
    ...
    with open(text_file, "w", encoding="utf-8") as f:
        ...
        if presentation_plan:
            ...
            f.write(
                f"Style Guidelines: {presentation_plan.style_guidelines.get('description', 'N/A')}\n\n"
            )
...
def load_last_checkpoint(output_dir: Path) -> str | None:
    """
...
def main(
    ...
    try:
        ...
        if cache and cache.exists(cache_key):
            ...
            paper_analysis = doc_analyzer.analyze_paper(
                pdf_path_obj, progress_callback=progress_callback
            )
            if cache:
        ...
        logger.info("✓ Checkpoint saved: paper_analysis (JSON + readable text)")
    ...
    try:
        ...
        presentation_plan = presentation_planner.create_plan(
            paper_analysis, pdf_path_obj, progress_callback=progress_callback
        )
        ...
        logger.info("✓ Checkpoint saved: presentation_planning (JSON + readable text)")
    ...
    slides = slide_generator.generate_slide_sequence(
        presentation_plan, pdf_path_obj, progress_callback=progress_callback
    )
    ...
    save_checkpoint(
        output_path,
        "slide_generation",
        data={"slides_generated": len(slides), "slide_info": slide_info},
        paper_analysis=paper_analysis,
        presentation_plan=presentation_plan,
    )
    logger.info("✓ Checkpoint saved: slide_generation (JSON + readable text)")
    ...
    try:
        ...
        progress_callback(
            ProgressStage.COMPLETE, 100, f"Successfully generated {len(slides)} slides!"
        )
        ...
        logger.info("✓ Final checkpoint saved: complete (JSON + readable text)")
...
""",
    ...
    parser.add_argument("--output", required=True, help="Output directory for generated slides")
    parser.add_argument(
        "--no-cache", action="store_true", help="Disable caching of intermediate results"
    )
    parser.add_argument(

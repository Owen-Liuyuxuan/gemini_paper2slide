...
async def get_results(job_id: str):
    ...
    if status.stage != ProgressStage.COMPLETE:
        raise HTTPException(
            status_code=400, detail=f"Generation not complete. Current stage: {status.stage.value}"
        )
    ...
    if not pdf_file.exists():
        raise HTTPException(status_code=404, detail="PDF file not found in output directory")

    return FileResponse(
        pdf_file, media_type="application/pdf", filename=f"presentation_{job_id}.pdf"
    )
...
async def get_slide_images(job_id: str):
    ...
    slides = [
        {"filename": slide.name, "url": f"/api/files/{job_id}/{slide.name}"}
        for slide in slide_files
    ]

    return JSONResponse({"job_id": job_id, "slides": slides, "count": len(slides)})
